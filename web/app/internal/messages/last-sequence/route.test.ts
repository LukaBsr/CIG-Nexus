...
describe("GET /internal/messages/last-sequence", () => {
  ...
  it("returns null for both when no messages exist", async () => {
    const response = await GET(new NextRequest(LAST_SEQUENCE_URL, { headers: SECRET_HEADERS }));
    const body = (await response.json()) as { lobby_seq: number | null; channel_seq: number | null };
    expect(body).toEqual({ lobby_seq: null, channel_seq: null });
  });
  ...
  it("returns the max seq per scope, independent of each other", async () => {
    ...
    const response = await GET(new NextRequest(LAST_SEQUENCE_URL, { headers: SECRET_HEADERS }));
    const body = (await response.json()) as { lobby_seq: number | null; channel_seq: number | null };
    expect(body).toEqual({ lobby_seq: 5, channel_seq: 2 });
  });
});
