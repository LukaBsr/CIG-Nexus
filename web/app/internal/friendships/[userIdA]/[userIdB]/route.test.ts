...
describe("DELETE /internal/friendships/:userIdA/:userIdB", () => {
  it("returns 401 without the shared secret", async () => {
    const request = new NextRequest("http://internal/x", { method: "DELETE" });
    const response = await DELETE(request, { params: Promise.resolve({ userIdA: "u_x", userIdB: "u_y" }) });
  ...
  it("removes an existing friendship", async () => {
    ...
    const response = await DELETE(request, {
      params: Promise.resolve({ userIdA: toUserWireId(a.id), userIdB: toUserWireId(b.id) })
    });
    expect(response.status).toBe(200);
    expect(await db.select().from(friendships)).toHaveLength(0);
  });

  it("returns 404 when no such friendship exists", async () => {
    const request = new NextRequest("http://internal/x", { method: "DELETE", headers: SECRET_HEADERS });
    const response = await DELETE(request, {
      params: Promise.resolve({
        userIdA: "u_00000000-0000-0000-0000-000000000000",
        userIdB: "u_00000000-0000-0000-0000-000000000001"
      })
    });
