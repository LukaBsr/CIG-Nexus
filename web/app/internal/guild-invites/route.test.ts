...
const URL = "http://internal/internal/guild-invites";
...
describe("POST /internal/guild-invites", () => {
  ...
  it("creates an invite", async () => {
    ...
    const request = new NextRequest(URL, {
      ...
      body: JSON.stringify({ guild_id: guild.guild_id, created_by: toUserWireId(owner.id) })
    });
    const response = await POST(request);
    expect(response.status).toBe(201);
    const body = (await response.json()) as { code: string; max_uses: null };
    expect(body.code).toBeTruthy();
    expect(body.max_uses).toBeNull();
  });

  it("returns 400 for a non-positive max_uses", async () => {
    const request = new NextRequest(URL, {
      method: "POST",
      headers: SECRET_HEADERS,
      body: JSON.stringify({ guild_id: "g_x", created_by: "u_x", max_uses: 0 })
    });
    expect((await POST(request)).status).toBe(400);
  });

  it("returns 400 for an invalid guild_id", async () => {
    const [owner] = await db
      .insert(users)
    ...
    const request = new NextRequest(URL, {
      ...
      body: JSON.stringify({
        ...
        created_by: toUserWireId(owner.id)
      })
    });
    expect((await POST(request)).status).toBe(400);
  });
...
describe("GET /internal/guild-invites", () => {
  it("returns 401 without the shared secret", async () => {
  ...
  it("returns the invites for the guild", async () => {
    const [owner] = await db
      .insert(users)
